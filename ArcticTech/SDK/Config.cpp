...
config_t config;
inline std::unique_ptr<CConfig> Config = std::make_unique<CConfig>();
...
void on_load() {
    ...
    if (!std::filesystem::exists(file_path)) {
        Utils::Print("[Sunset] config doesnt exists: %s\n", name);
        return;
    }
...
void CConfig::Init() {
    ...
    add(config.ragebot.aimbot.hideshots_options);
    ...
    add(config.antiaim.angles.defensive);
    ...
    add(config.misc.ui_group.indicators);
