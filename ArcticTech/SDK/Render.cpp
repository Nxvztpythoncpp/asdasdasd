...
void CRender::gradient_text(const std::string& text, const Vector2& pos, Color start_color, Color end_color, float alpha, D3DXFont* font, int flags) {
	const float text_width = Render->CalcTextSize(text, font).x;
	const float step = text_width / text.length();
	float x = pos.x;

	for (size_t i = 0; i < text.length(); i++) {
		const float progress = i / float(text.length() - 1);
		Color current_color = Color(
			std::lerp(start_color.r, end_color.r, progress),
			std::lerp(start_color.g, end_color.g, progress),
			std::lerp(start_color.b, end_color.b, progress),
			alpha
		);

		std::string current_char(1, text[i]);
		this->Text(current_char, Vector2(x, pos.y), current_color, font, flags);
		x += step;
	}
}
...
inline std::unique_ptr<CRender> Render = std::make_unique<CRender>();